...
export function IdleScreen({
  ...
  const [showManualRecoverInput, setShowManualRecoverInput] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (!showRecover) setShowManualRecoverInput(false);
  }, [showRecover]);

  useEffect(() => {
    const input = fileInputRef.current;
    if (!input) return;
    const onCancel = () => setShowManualRecoverInput(true);
  ...
  const handleRecoverFile = (file: File) => {
    ...
    <div className="space-y-4 p-4">
      ...
      {!showRecover ? (
        <>
          ...
          <button
            onClick={() => {
              setShowManualRecoverInput(false);
              onShowRecover();
              openFilePicker();
            }}
            className="block w-full text-center text-sm text-neutral-500 hover:text-neutral-900 underline underline-offset-2 transition-colors dark:text-neutral-600 dark:hover:text-neutral-100"
          >
            Recover Wallet
          </button>
        </>
        ...
        <div className="space-y-3">
          <button
            ...
            className="flex items-center justify-center gap-1.5 w-full py-2.5 text-sm font-medium rounded-xl border border-dashed border-neutral-300 text-neutral-500 hover:border-neutral-400 hover:text-neutral-800 cursor-pointer transition-colors dark:border-neutral-700 dark:text-neutral-400 dark:hover:border-neutral-600 dark:hover:text-neutral-200"
          >
